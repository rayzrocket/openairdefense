...
cameraProcess = sp.Popen(videoCmd, stdout=sp.PIPE, stderr=sp.PIPE, bufsize=((w*h)+20)) # start the camera and set min buffer size
...
#     print('width = ',width,'  Bandpass size = ',BandPass.shape)
#     print('Time= ',(time.time()-stime),'SelctPtVal = ',SelctPtVal,' clickflag= ',clickflag)
#     print('BandPass MAX = ',np.amax(BandPass),'  BandPass min = ',np.amin(BandPass))
