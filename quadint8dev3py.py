...
#    start=time.time()
#    hs=(tbox[2]-tbox[0])/2#box width div by 2
...
#    BandPass=np.uint8(BandPass)#doing this forces imshow not to scale values, ie: max maybe just 4

    
#    LoPass=np.uint8(LoPass)
#    if np.amax(LoPass)<1:
#        LoPass[int(width/2),int(width/2)]=1
    print('SelctPtValB = ',SelctPtValB)
...
##    HiPass=HiPassBVal+subframeB#all values less than x-val become zero
#    HiPass=np.clip(HiPass,0,255)
#    HiPass=np.uint8(HiPass)
#    if np.amax(HiPass)<1:
#        HiPass[int(width/2),int(width/2)]=1
#    print('SelctPtValB = ',SelctPtValB)
#    print('HiPass MAX = ',np.amax(HiPass))
#    print('HiPass min = ',np.amin(HiPass))
#    sumBx=np.sum(HiPass,axis=0)
#    sumBy=np.sum(HiPass,axis=1)
#    plt.plot(HiPass[int(width/2),:])
#    plt.plot(HiPass[:,int(width/2)])
#    plt.show()
    
#    sBx=np.sum(subframeB,axis=0)
#    if sBx.size > 1:
#        sBxMin=np.argmin(sBx)#pixel x location where min occurs
#        sBy=np.sum(subframeB,axis=1)
#        if sBy.size > 1:
#            sByMin=np.argmin(sBy)
#            tpoint=(int(sBxMin+bx1),int(sByMin+by1))
    #
    return tpoint,width,subframe,box,BandPass,sumBx,sumBy#subframe,subframeB,subframeG,subframeR#bbox

def main():
#    global tbox#required for mouse call function def return_mouse_click
    global tpoint
#    global varBreak
    varBreak=0#for exiting the while loop
...
#    cv2.namedWindow("ShowbboxRed", cv2.WINDOW_NORMAL)
#    cv2.namedWindow("ShowbboxBlue", cv2.WINDOW_NORMAL)
#    cv2.namedWindow("ShowbboxGreen", cv2.WINDOW_NORMAL)
...
#    startwidth=100
    ...
    width=60
#    sbox = (int(tbox[0]-(tbox[2]-tbox[0])),int(tbox[1]-(tbox[3]-tbox[1])),int(tbox[0]+(tbox[2]-tbox[0])),int(tbox[1]+(tbox[3]-tbox[1])))
    n=int(0)
    start=time.time()
    while loop==1:#*******************************************
#        start=time.time()
#        for x in range(0,19):
        ...
        tpoint,width,subframe,box,BandPass,sumBx,sumBy= simple_tracker(tpoint,width,frame)#
