...
#    fig=plt.figure()
    ...
    cv2.namedWindow("CCFRAME", cv2.WINDOW_NORMAL)#
    cv2.namedWindow("tester", cv2.WINDOW_NORMAL)#
    cv2.namedWindow("movedframe", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder0", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder1", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder2", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder3", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder4", cv2.WINDOW_NORMAL)#
#    cv2.namedWindow("CCBlobfinder5", cv2.WINDOW_NORMAL)#

#    cv2.namedWindow("ccframe", cv2.WINDOW_NORMAL)#
    ccframe=ccframe
    print(np.amax(ccframe),' ccframe max val')
    print(np.amin(ccframe),' ccframe min val')
#    cv2.imshow("ccframe",ccframe)
    rf=(ccframe.shape[1])#
    cf=(ccframe.shape[0])
...
#                    ncurrent=int(ccframe[r,c]+99)#ncurrent is a positive value used for indexing the neg marker value
#                    if ccframe[r,c+1]!=0: #look east, is it nonzero, if so...
#                    if ccframe[r,c+1]!=ccframe[r,c]:#if it's NOT already labled to current group..
                    #Look East
                    if ccframe[r,c+1] < m:#if it has been labeled in prior group...
                        ...
                        movedframe=blob[:,:,ncurrent]
                        cv2.imshow('movedframe',movedframe)
                        cv2.waitKey(250)
        ...
        if m==0:
            print('blob count exceeds 99')
            break
#    print(np.amax(blob[:,:,1]),' max blob value')
    cv2.waitKey(1000)
    ...
    if n==-1: #handles when there are NO blobs found
        ...
        print(n,' n total blobs generated enter rearrange loop')
        ...
        for i in range(0,(n+1)): #remove any blank blob dims and resort
            ...
            if np.any(blob[:,:,i]):#check for non zero values
                ...
                print('nonzero blob #',i,' added to ccframe2')
                cv2.imshow("CCFRAME",ccframe2)
                cv2.waitKey(2000)
            else:
                blob=np.delete(blob,i,axis=2)#destroy this 'i' blob frame
                print('blob #',i,' destroyed')
                x=x+1
        blob2=np.uint8(blob)#(blob/np.amax(blob))*254)#I can't recall why I did this, maybe something to do with emptying blob above and rebuilding ccframe2 below
        print(n-x,' =n-x total blobs generated and the blobfinder algo is complete')
#        b0=np.uint8(blob2[:,:,0])
#        b1=np.uint8(blob2[:,:,1])
#        b2=np.uint8(blob2[:,:,2])
#        cv2.imshow("CCBlobfinder0",b0)
#        cv2.imshow("CCBlobfinder1",b1)
#        cv2.imshow("CCBlobfinder2",b2)
...
#        plt.show()
        width=newwidth
#        cv2.waitKey(5)#time.sleep(0.050)#if using varBreak , then use this to let .imshow update
        stop=time.time()
        print(int(1/(stop-start)),' Hz')
#        freq=20/(stop-start)
#        print('Frequency %.1fHz' % (freq))
#        #since .waitKey takes so long, we use double mouse click on image to set the 'break' variable
